...
actual class DatabaseFactory private constructor() {
    ...
    private var database: AndroidU1SlicerDatabase? = null

    actual suspend fun createDatabase(): AppDatabase = withContext(Dispatchers.IO) {
        val ctx = context ?: throw IllegalStateException("Context not set. Call initialize() first.")
        val sqlDriver = AndroidSqliteDriver(
            schema = com.u1.slicer.shared.database.U1SlicerDatabase.Schema,
            context = ctx,
            name = "u1_slicer.db"
        )
        driver = sqlDriver
        val sqlDelightDb = com.u1.slicer.shared.database.U1SlicerDatabase(sqlDriver)
        val db = AndroidU1SlicerDatabase(sqlDelightDb)
        database = db
        getLogger().d("DatabaseFactory", "Database created using SQLDelight on Android")
        db
    }
...
/**
 * Android implementation using SQLDelight
 */
class AndroidU1SlicerDatabase(private val sqlDriver: com.u1.slicer.shared.database.U1SlicerDatabase) : AppDatabase {
...
private fun Filament_profiles.toFilamentProfile() = FilamentProfile(
    ...
    printSpeed = print_speed?.toFloat() ?: 0f,
    retractLength = retract_length?.toFloat() ?: 0f,
    retractSpeed = retract_speed?.toFloat() ?: 0f,
    color = color ?: "#808080",
